...
from app.services.lookup_service_clinvar_distribution import (
    clinvar_distribution_runtime_path,
    clinvar_distribution_store,
)
...
def _fixture_curated_variant_page(
    ...
    *,
    ...
    next_cursor = (
        _encode_curated_cursor(selected[-1].gnomad_variant_id) if has_more and selected else None
    )
    ...
    *,
    ...
    try:
        with sqlite3.connect(index_path) as connection:
            ...
            total = int(
                connection.execute(
                    "select count(*) from clinvar_gene_distribution_variant " f"where {base_where}",
