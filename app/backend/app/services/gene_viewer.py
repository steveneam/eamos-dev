...
class SourceBackedGeneViewerProvider:
    ...
    def viewer_bundle(self, payload: GeneViewerRequest) -> SourceBackedViewerBundle:
        ...
        _hydrate_response_with_source_protein_track(
            response=response,
        ...
        if "alphamissense" in payload.tracks:
            _hydrate_response_with_alphamissense_heatmap(
                response=response,
                variant=variant,
                transcript_source=transcript_source,
...
def _hydrate_response_with_source_protein_track(
    *,
    ...
    if protein_annotation_service is None:
        return
    try:
        coding_dna = _coding_dna_from_source_transcript(
            transcript_source,
...
def _coding_dna_from_source_transcript(
    source: SourceTranscriptModel,
    *,
    sequence_for_exon: Callable[[SourceTranscriptExon], str],
) -> str | None:
...
def _hydrate_response_with_alphamissense_heatmap(
    *,
    response: GeneViewerResponse,
    variant: VariantProjection,
    transcript_source: SourceTranscriptModel,
    ...
    if alphamissense_adapter is None:
        ...
        return
    try:
        coding_dna, genomic_positions = _coding_dna_and_positions_from_source_transcript(
            transcript_source,
            sequence_for_exon=sequence_for_exon,
        )
    except GeneViewerError as exc:
...
def _coding_dna_and_positions_from_source_transcript(
    source: SourceTranscriptModel,
    *,
    sequence_for_exon: Callable[[SourceTranscriptExon], str],
) -> tuple[str, tuple[int, ...]]:
    ...
    *,
    ...
    response = GeneViewerResponse(
        ...
        provenance=ViewerProvenance(
            sources=[
                *_curated_fixture_provenance_sources(
                    record=record,
...
def _full_locus_projection_intervals(
    ...
    utr5_length = _fixture_int_or_none(record.get("utr5_length")) or 0
    cds_length = _fixture_int_or_none(record.get("cds_length")) or 0
...
def _full_locus_utr3_intervals(
    *,
    ...
    utr3_length = _fixture_int_or_none(record.get("utr3_length")) or 0
    if utr3_length <= 0:
        return []
    gene_start = int(record["gene_start"])
    gene_end = int(record["gene_end"])
...
def _build_full_locus_sequence(
    *,
    ...
    strand = _schema_strand(str(record.get("strand") or "unknown"))
    for exon in _record_exons(record):
        sequence = str(exon.get("sequence") or _placeholder_sequence(exon)).upper()
...
def _raise_full_gene_not_hydrated() -> None:
    code = unsupported_input_warning("full_gene")
    raise GeneViewerError(
        code=code,
        message=(
