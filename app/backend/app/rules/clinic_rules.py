...
class ClinicRules:
    def evaluate(self, payload: DecisionInput) -> DecisionOutput:
        ...
        subject = "The patient"
        variant_summary = (
            payload.variant_summary[0] if payload.variant_summary else payload.case_title
        )
        clinical_context = (
            payload.clinical_findings or payload.patient_context or "the reported clinical context"
        )
        max_splice = max(
        ...
        if max_splice >= 0.5:
            ...
            splice_interpretation = (
                "scores do not show a strong predicted splice-disruption signal."
            )
