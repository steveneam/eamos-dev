...
def test_eamos_search_input_cli_prints_source_bundle_for_file(tmp_path, capsys) -> None:
    ...
    assert (
        output["results"][1]["coordinate_resolution_audit"]["resolver_path"] == "submitted_genomic"
    )
    assert (
        output["results"][1]["coordinate_resolution_audit"]["used_clinvar_for_coordinates"] is False
    )
    assert (
