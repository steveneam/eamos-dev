...
def test_create_run_with_multiple_reports(auth_client: TestClient, pdf_bytes: bytes) -> None:
    ...
    response = auth_client.post(
        "/api/v1/runs", json={"patient_id": "RP-001", "report_ids": report_ids}
    )
...
def test_patient_blocked_run_status_is_marked_blocked(
    auth_client: TestClient, pdf_bytes: bytes
) -> None:
    report_ids = [_upload_report(auth_client, pdf_bytes, report_kind="patient")]
    response = auth_client.post(
        "/api/v1/runs", json={"patient_id": "RP-002", "report_ids": report_ids}
    )
    assert response.status_code == 200
...
def test_run_includes_evidence_without_nulls(auth_client: TestClient, pdf_bytes: bytes) -> None:
    ...
    response = auth_client.post(
        "/api/v1/runs", json={"patient_id": "RP-003", "report_ids": [report_id]}
    )
    assert response.status_code == 200
    body = response.json()
...
def test_run_surface_degraded_evidence_when_tool_falls_back(
    ...
    monkeypatch.setattr(
        app.state.workflow_service.tool_registry["clinvar"], "get_evidence", fake_fallback_result
    )

    response = auth_client.post(
        "/api/v1/runs", json={"patient_id": "RP-004", "report_ids": [report_id]}
    )
    assert response.status_code == 200
    body = response.json()
...
def test_run_applies_llm_rewrite_only_to_narrative_fields(
    ...
    response = auth_client.post(
        "/api/v1/runs", json={"patient_id": "RP-005", "report_ids": [report_id]}
    )
    assert response.status_code == 200
    body = response.json()
    ...
    assert "Ravi" not in body["report_payload"]["patient_context"]
    assert (
        body["report_payload"]["ai_clinical_summary"]
        == "Rewritten clinical summary for clinician review."
    )
    assert (
        body["report_payload"]["expanded_evidence"]
        == "Rewritten evidence narrative preserving source findings."
    )
    assert body["report_payload"]["variant_summary_rows"][0]["gene"] == "RPE65"


def test_run_falls_back_cleanly_when_llm_draft_fails(
    auth_client: TestClient, app, pdf_bytes: bytes
) -> None:
    report_id = _upload_report(auth_client, pdf_bytes)
    ...
    response = auth_client.post(
        "/api/v1/runs", json={"patient_id": "RP-006", "report_ids": [report_id]}
    )
    assert response.status_code == 200
    body = response.json()
