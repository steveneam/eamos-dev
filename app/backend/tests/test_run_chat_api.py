...
def _create_run(client: TestClient, pdf_bytes: bytes, patient_id: str = "RP-CHAT-001") -> str:
    ...
    response = client.post(
        "/api/v1/runs", json={"patient_id": patient_id, "report_ids": [report_id]}
    )
...
def test_run_chat_returns_grounded_answer_with_citations(
...
def test_run_chat_returns_unknown_for_unsupported_question(
    auth_client: TestClient, app, pdf_bytes: bytes
) -> None:
    ...
    response = auth_client.post(
        f"/api/v1/runs/{run_id}/chat", json={"question": "What medication should be started?"}
    )
    assert response.status_code == 200
    body = response.json()
...
def test_run_chat_returns_404_for_unknown_run(auth_client: TestClient, app) -> None:
    ...
    response = auth_client.post(
        "/api/v1/runs/run_missing/chat", json={"question": "What does ClinVar say?"}
    )
    assert response.status_code == 404


def test_run_chat_handles_degraded_runs(
    ...
    run_response = auth_client.post(
        "/api/v1/runs", json={"patient_id": "RP-CHAT-004", "report_ids": [report_id]}
    )
    ...
    response = auth_client.post(
        f"/api/v1/runs/{run_id}/chat", json={"question": "What does the evidence show?"}
    )
    assert response.status_code == 200
