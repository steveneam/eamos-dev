...
function emit({ viewBox, landPath, regions }) {
  const regionLines = regions
    .map((r) => `  { group: '${r.group}', countries: ${r.countries}, d: ${JSON.stringify(r.d)} },`)
    .join('\n')
...
function main() {
  ...
  for (const group of GROUP_ORDER) {
    ...
    regions.push({ group, countries: members.length, d })
  }
  ...
  for (const r of regions) {
    console.log(`    ${r.group}  ${String(r.countries).padStart(2)} countries  ${r.d.length} path chars`)
  }
