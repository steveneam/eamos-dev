...
- **Codex:** IDLE @ 2026-06-11 21:52 +10:00 - Combined Workbench/PubMed integration shipped as `43f6e92` and pushed to `origin/main`; Render SG deploy `dep-d8l9tfa8qa3s738k78tg` is live on `43f6e92`; Vercel production deployment `eamos-141hgwceg-steven-eamegdool-s-projects.vercel.app` is Ready and aliased to `eamos-dev.vercel.app`. Live probes passed: SG `/healthz` 200; SG+Vercel provider-cache report CRISPR off-target `configured_provider=auto`, `status=mock_fallback`, `indexed_sqlite.ready=false`, `request_time_supabase_search=false`; SG+Vercel lookup/publications POST return 200 for `USH2A:c.2276G>T` (`total_count=190`, five rows); `/report?demo=1` on Vercel returns 200. `.claude/settings.json` and `codex-workbench-temp.md` remain intentionally uncommitted. No generated/private CRISPR off-target SQLite/genome/index artifact was staged.
...
UNLOCKED - 2026-06-11 21:52 +10:00 - Codex (combined Workbench/PubMed integration committed+pushed as 43f6e92; Render SG and Vercel verified; CRISPR indexed provider remains env-gated)
...
Section last edited: 2026-06-11 21:52 +10:00 - Codex. Detailed history is in PROGRESS.md and commit messages.

**Latest Codex update (2026-06-11 21:52 +10:00 - Codex):**
Steven approved one combined integration commit for the parallel Workbench and
PubMed/report slices. Codex committed and pushed `43f6e92` to `origin/main`,
then triggered and verified Render SG plus Vercel production.

Completed:
- Workbench: live provider wiring, Primer3 thermodynamic/placement fields,
  CRISPR ssODN genomic coordinate, optional local SQLite CRISPR off-target
  index source/CLI, provider-cache health gates, contract mirrors, fixed-step
  gene-viewer zoom, and durable Workbench deploy notes in
  `docs/workbench-backend-wiring/spec.md`.
- PubMed/report: exact variant no-hit behavior with separate gene-scope count,
  LitVar2 PMID hydration, PubMed corpus budget and LitVar operator CLIs/tests,
  report publication timeline interactions, and count-only gene-scope UI.
- Shared docs/graph: `PROGRESS.md`, `agent_handoff/CURRENT.md`, and refreshed
  `graphify-out/*` included in the combined commit.

Verification:
- Local combined-tree checks passed: Workbench/health pytest; PubMed/health/tool
  pytest; backend Ruff; scoped Black checks; CRISPR index CLI help;
  `app/frontend npx tsc --noEmit`; `app/web npx tsc --noEmit`; cached
  diff-check.
- Render SG deploy `dep-d8l9tfa8qa3s738k78tg` is live on commit `43f6e92`.
- Vercel production deployment
  `eamos-141hgwceg-steven-eamegdool-s-projects.vercel.app` is Ready and aliased
  to `https://eamos-dev.vercel.app`.
- Live probes passed: SG `/healthz`; SG+Vercel provider-cache; SG+Vercel
  `POST /api/v1/lookup?refresh=true&include_lazy_sections=true`; SG+Vercel
  `POST /api/v1/lookup/publications?refresh=true`; and Vercel
  `/report?demo=1`.

Guardrails:
- Keep `CRISPR_OFFTARGET_PROVIDER=auto` in production unless Render has a real
  local `CRISPR_OFFTARGET_INDEX_PATH` and provider-cache health reports the
  index ready. Current live status is `mock_fallback`, `indexed_sqlite.ready=false`,
  `request_time_supabase_search=false`.
- No generated/private CRISPR off-target SQLite/genome/index artifact was
  staged.
- `.claude/settings.json` and `codex-workbench-temp.md` remain intentionally
  uncommitted local files.
- Supabase corpus upload/table/vector work remains budget-gated; no Supabase
  mutation happened in this integration.

**Latest resume prompt:**
```text
# Resume prompt - 2026-06-11 21:52 +1000 - Codex combined Workbench/PubMed integration shipped
Eamos. Continue in D:\eamos on Windows/PowerShell only. Read CODEX.md, AGENTS.md, agent_handoff/README.md, agent_handoff/CURRENT.md, agent_handoff/RISKS.md, PROGRESS.md, docs/workbench-backend-wiring/spec.md, and docs/pubmed-local/plan.md, then run git status --short --branch.

Delta: combined Workbench/PubMed integration shipped as `43f6e92` and was pushed to origin/main. Render SG deploy `dep-d8l9tfa8qa3s738k78tg` is live on `43f6e92`; Vercel production deployment `eamos-141hgwceg-steven-eamegdool-s-projects.vercel.app` is Ready and aliased to `eamos-dev.vercel.app`.

Verification already passed: local Workbench/health pytest, PubMed/health/tool pytest, backend Ruff, scoped Black, CRISPR index CLI help, app/frontend tsc, app/web tsc, diff-check; live SG health/provider-cache/lookup/publications and Vercel provider-cache/lookup/publications/report demo probes.

Guardrails: keep `CRISPR_OFFTARGET_PROVIDER=auto` unless Render has a real local `CRISPR_OFFTARGET_INDEX_PATH` and provider-cache says the index is ready; no generated/private CRISPR index artifacts are committed; `.claude/settings.json` and `codex-workbench-temp.md` remain local/uncommitted; no Supabase corpus mutation without renewed budget approval.
End clear-safe.
```
