...
- **Codex:** IDLE @ 2026-06-08 14:32 +1000 - PubMed-local v4 + PubTator converter committed as `59feb38`, pushed to `origin/main`, deployed to SG Render `dep-d8j43or7uimc73bb1nn0`, and live-verified through SG + Vercel. Claude `app/web/components/report/**` WIP remains unstaged/off-limits. Next: operator LitVar export/materializer path or production PubMed/PubTator asset seeding plan, preserving disabled-by-default local lane and no-startup-download policy.
...
Section last edited: 2026-06-08 14:07 +1000 - Codex. Detailed history is in PROGRESS.md and commit messages.

**Latest Codex update (2026-06-08 14:07 +1000 - Codex):**
Steven explicitly authorized Claude coordination, commit, push, and deploy for
the PubMed-local backend lane. Coordination check: Claude is idle in Active
Status, no fresh handoff/shared lock is held, and Claude-owned
`app/web/components/report/**` WIP is excluded from staging.

Completed:
- PubMed-local schema/manifest v4 adds local `pubmed_literature_edge`,
  operator PubTator/LitVar edge JSONL inputs, source provenance/counters, seed
  pre-scan, and EP-VLEx-compatible `pubtator` / `litvar2_snippet` enrichment.
- Added `python -m app.cli.eamos_pubmed_pubtator_edges`, a no-network
  operator CLI that converts NCBI PubTator flat files into the existing
  PubMed-local PubTator edge JSONL shape.
- Preserved disabled-by-default local PubMed, live E-utilities fallback,
  `refresh=true` live bypass, no startup/request-time materialization, no
  frontend contract change, no Supabase mutation, and no raw-path/abstract
  leakage in CLI/preflight/health reports.

Verification:
- `python -m pytest tests\test_pubmed_pubtator_edges.py -q`
- `python -m pytest tests\test_pubmed_local.py tests\test_pubmed_pubtator_edges.py -q`
- `python -m pytest tests\test_pubmed_local.py tests\test_pubmed_pubtator_edges.py tests\test_health_api.py tests\test_publication_literature.py tests\test_lookup_section_fetch_contract.py tests\test_variant_cache.py tests\test_frontend_contract.py -q`
- `python -m ruff check app\services\pubmed_local.py app\cli\eamos_pubmed_local_materialize.py app\cli\eamos_pubmed_local_preflight.py app\cli\eamos_pubmed_pubtator_edges.py tests\test_pubmed_local.py tests\test_pubmed_pubtator_edges.py app\api\routes\health.py tests\test_health_api.py`
- `python -m black --check --target-version py310 app\services\pubmed_local.py app\cli\eamos_pubmed_local_materialize.py app\cli\eamos_pubmed_local_preflight.py app\cli\eamos_pubmed_pubtator_edges.py tests\test_pubmed_local.py tests\test_pubmed_pubtator_edges.py app\api\routes\health.py tests\test_health_api.py`
- `python -m py_compile app\cli\eamos_pubmed_pubtator_edges.py app\services\pubmed_local.py app\cli\eamos_pubmed_local_materialize.py app\cli\eamos_pubmed_local_preflight.py app\tools\pubmed.py app\api\routes\health.py`
- `python -m graphify update .` passed; graph HTML skipped because graph has
  11,829 nodes.

Commit/deploy state:
- Committed as `59feb38` (`feat(pubmed): materialize local literature edges`)
  and pushed `3129834..59feb38` to `origin/main`.
- SG Render deploy `dep-d8j43or7uimc73bb1nn0` is live on commit
  `59feb38a6e0d98ae515b2244a04459acfb74c194`.
- Live verification passed: SG `/healthz`; SG provider-cache with
  `pubmed_local enabled=false/status=db_missing/startup_download_allowed=false`;
  SG and Vercel `/api/v1/lookup?refresh=true` for `USH2A c.2276G>T`; SG and
  Vercel `/api/v1/lookup/publications?refresh=true` returned matching
  paginated publication totals (`total=190`, first PMID `41892280`).
- Do not stage or edit Claude FE/report WIP:
  `app/web/components/report/{GeneDiseaseBlock,MaveFunctionalBlock,MolecularContextBlock,PubMedSection,PublicationTimelineChart,PublicationsCallout,ReportClient,ProteinTrack}.tsx`.
- Never use `git add -A`.

**Clear-safe:** yes. PubMed-local backend commit is pushed and live on SG;
handoff is updated; only Claude-owned FE/report WIP remains dirty locally.

**Latest resume prompt:**
```text
# Resume prompt - 2026-06-08 14:32 +1000 - Codex PubMed-local deployed
...
Delta: PubMed-local v4 edge ingestion plus PubTator flat-file-to-edge-JSONL converter shipped in `59feb38`, pushed to origin/main, deployed to SG `dep-d8j43or7uimc73bb1nn0`, and live-verified via SG+Vercel health/lookup/publications.
Next: choose the operator LitVar export/materializer path or production PubMed/PubTator asset-seeding plan.
Guardrails: do not edit/stage Claude app/web report WIP; no git add -A; preserve live fallback/refresh and no-startup-download policy.
End clear-safe.
```
