...
- **Codex:** IDLE @ 2026-06-12 00:26 +10:00 - Workbench live-wiring quick wins implemented locally, plus Steven's rendered gene-viewer drag/spacing feedback. Completed Task 1 primer live-field UI consumption and Task 6 observed-only CRISPR/TIDE-style outcomes route; TIDE sites were used as output references only, with Eamos using its own local/provider-backed route rather than live-calling public TIDE pages. Viewer rows now fill the fullscreen canvas, row-level pointer-captured drag selection starts from anywhere in the sequence line, continues off-line/across rows at requestAnimationFrame cadence, and the Sequence top gutter/zoom clearance is fixed. No downloads, no Render/Supabase/Vercel/env/provider/source-asset changes. Steven then asked to coordinate the commit and push with the other Codex next session, so the index is intentionally unstaged and this slice remains uncommitted. Verification: backend focused pytest + Ruff + touched-file Black passed; web/frontend TypeScript passed; browser verified desktop/mobile/drag/spacing with no runtime/network errors; `git diff --check` passed; `python -m graphify update .` passed. Full backend Black still fails on unrelated pre-existing files. Dev server left running at `http://localhost:3000/workbench`.
...
Section last edited: 2026-06-12 00:12 +10:00 - Codex. Detailed history is in PROGRESS.md, commit messages, and `docs/workbench-live-wiring/`.

**Latest Codex update (2026-06-12 00:12 +10:00 - Codex):**
Steven approved the Workbench live-wiring quick wins, then live-reviewed the
gene/sequence viewer. Codex implemented the local changes and stopped before
downloads, provider/env/runtime mutations, deploys, commits, or pushes.

Completed:
- Task 1: primer result cards now consume live Primer3/provider fields for
  placement, genomic coordinates, amplicon, and thermodynamic/self-complement
  values when the backend supplies them. Fallback/illustrative states remain
  explicitly labelled.
- Task 6: added an observed-only CRISPR/TIDE-style backend route,
  `POST /api/v1/crispr/tide`, accepting control and edited AB1 uploads plus
  `cut_site_index`, returning an indel spectrum, editing-efficiency estimate,
  fit proxy, quality notes, and source label. This is intentionally not NKI
  TIDE/TIDER NNLS/p-value/Lindel output.
- CRISPR outcomes UI and both frontend/backend type mirrors now consume the
  source-backed TIDE-style response, fall back only on route-missing/transport
  conditions, and surface 422/503 errors honestly.
- TIDE/NKI and TIDE Genetics were used as output references only. Eamos should
  use its own local/provider-backed route unless a documented licensed API is
  deliberately added later.
- Gene viewer polish from Steven's screenshots: sequence rows now fill the
  fullscreen canvas width; top gutter is increased; the zoom pill clears the
  first row; row-level drag selection starts from anywhere on a sequence row,
  including whitespace/between bases; pointer capture keeps selection active
  while the cursor is off-line; requestAnimationFrame throttling removes the
  per-base hover/update lag.
- `python -m graphify update .` refreshed `graphify-out/graph.json`,
  `graphify-out/GRAPH_REPORT.md`, and `graphify-out/manifest.json`.
...
- Browser-verified `http://localhost:3000/workbench` on desktop and mobile:
  viewer rows fill the available width, sequence top gap measured 76px, zoom
  clearance measured 16px, row-whitespace drag, reverse drag, and off-line/
  cross-row drag all selected correctly, with no runtime/network failures.
- `git diff --check` passed with line-ending warnings only.

Guardrails:
- No downloads, no Render/Supabase/Vercel/env/provider/source-asset changes, no
  commit/push.
- After wrap, Steven asked to coordinate the commit and push with the other
  Codex next session. The index was unstaged with `git restore --staged -- .`.
  Next session should coordinate first, then stage explicit paths only. For
  `app/web/lib/api.ts`, stage only the Workbench `analyzeTide` hunk; leave the
  unrelated `fetchLookupSections` retry/signal hunk out unless the other Codex
  owns or approves it. Also leave `.claude/settings.json`,
  `codex-workbench-temp.md`, and `app/web/components/report/LazySection.tsx`
  out of this Workbench commit unless ownership changes.
- Keep `CRISPR_OFFTARGET_PROVIDER=auto` in production unless Render has a real
  local `CRISPR_OFFTARGET_INDEX_PATH` and provider-cache health reports
  `indexed_sqlite.ready=true`.
- Do not commit generated/private CRISPR off-target SQLite/genome/index
  artifacts.
- Dev server was left running at `http://localhost:3000/workbench`.
- Existing unrelated dirty worktree entries remain untouched.

**Latest resume prompt:**
```text
# Resume prompt - 2026-06-12 00:12 +1000 - Codex Workbench live-wiring quick wins and gene-viewer drag polish

Eamos. Continue in D:\eamos on Windows/PowerShell only. Read CODEX.md, AGENTS.md, agent_handoff/README.md, agent_handoff/CURRENT.md, agent_handoff/RISKS.md, PROGRESS.md, docs/workbench-live-wiring/analysis.md, docs/workbench-live-wiring/spec.md, and docs/workbench-live-wiring/plan.md, then run git status --short --branch.

Delta: Steven approved the Workbench live-wiring quick wins. Implemented Task 1 primer live-field UI consumption and Task 6 observed-only CRISPR/TIDE-style outcomes route; updated web/frontend mirrors and CRISPR outcomes UI; polished the gene viewer so fullscreen sequence rows fill width, top spacing clears the zoom control, and drag selection starts anywhere in a sequence row, continues off-line/across rows, and is requestAnimationFrame-throttled. TIDE/NKI and TIDE Genetics are output references only; do not live-call those public sites unless a licensed documented API is added deliberately.

Verification: backend focused Workbench/frontend-contract pytest, Ruff, touched-file Black, web/frontend TypeScript, browser desktop/mobile drag/spacing checks, git diff --check, and graphify update all passed. Full backend Black still fails on unrelated pre-existing files.

Guardrails: no downloads, no Render/Supabase/Vercel/env/provider/source-asset changes. Commit/push is deferred: Steven asked to coordinate it with the other Codex next session. Current index is intentionally unstaged. Stage explicit paths only; for app/web/lib/api.ts include only the Workbench analyzeTide hunk, not the unrelated fetchLookupSections retry/signal hunk. Leave .claude/settings.json, codex-workbench-temp.md, and app/web/components/report/LazySection.tsx out unless the other Codex owns/approves them. Keep CRISPR_OFFTARGET_PROVIDER=auto until a real CRISPR_OFFTARGET_INDEX_PATH exists on Render and provider-cache reports indexed_sqlite.ready=true. Dev server is running at http://localhost:3000/workbench.
End clear-safe.
```
