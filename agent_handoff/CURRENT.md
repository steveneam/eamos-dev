...
- **Claude:** ACTIVE @ 2026-06-07 00:37 +1000 — **Align v2 SHIPPING (Steven-directed commit/push/deploy). Align tool fully rebuilt this session into a Benchling/FinchTV-style aligner; FE-only `app/web/**`, NO contract/backend change (client-side ABIF parse, mock-first).** Steven reversed the A/B-subject Slice-2 model mid-session → new model = **one reference/template + N Sanger reads**, each read = a single shared-coordinate `AlignedTrace` (ref row + read row + chromatogram on ONE column grid: 1 base = 1 peak, 1 compression/intensity control + 1 scroll move bases AND trace together; base-coloured A/C/G/T matching the trace; mismatch/gap bands through all rows; queried-variant column ringed; sticky REF/file-name row labels). Features: client **ABIF parser** (`abif-parser.ts`), **auto-orient** (best of fwd/rev-comp on import), **Q-trim** (sliding-window Mott-ish, toggle Full-read), **quality-aware mismatches** (real vs low-Q via Phred), **het double-peak** detect (conservative: ratio≥0.5 + amplitude floor + Q-gate), **Find sequence** motif search (fwd+rev-comp, ◀▶, jump+highlight), **Mismatches/Gaps independent chip toggles**, **clickable diff chips → jump+highlight in BOTH the alignment and the chromatogram** (shared activeCol), per-trace intensity/compression controls (movement-revealed, top-right). Reference import: paste + FASTA now; **accession-search = Codex**. Files (MINE, staged at this commit — FE align ONLY): `app/web/components/workbench/align/{AlignPanel,AlignedTrace,PairwiseView,ReadRow}.tsx` + `{abif-parser,read-model}.ts` + `app/web/components/workbench/workbench.css` (deleted SubjectSlot/AlignResults/Chromatogram/align-subjects — the A/B-slice files, never committed). Gates: app/web tsc 0-err, lint 0-err/6-warn baseline. Browser-verified with Steven's real RPE65 VUS1 `.ab1` (multiple reads stack; peaks line up 1:1 under bases; auto-orient + Q-trim + het + chip-jump-in-trace + search all confirmed). **⚠ Codex has uncommitted `app/backend/**` WIP in the shared tree — I am NOT touching/staging it; I stage explicit FE pathspecs only.**
...
- **Codex:** INACTIVE @ 2026-06-06 15:49 +1000 - Backend-only handoff accepted from Steven and completed. Pushed `4e77a12` (`fix(backend): harden lazy sections and library index`) to `origin/main`, then deployed SG Render service `srv-d8ctvoh9rddc73a27nb0` via API deploy `dep-d8hr8nu7r5hc73c73230`; deploy is live on commit `4e77a12cd9db4e5740c34dad6eea6f1f99824a50`. Supabase follow-up migration `20260606051914_variant_library_folder_fk_index.sql` is applied and verified: `public.saved_variant(folder_id)` has partial FK-covering index `idx_saved_variant_folder_id`; security advisor clean; prior unindexed-FK performance lint gone; only expected immediate unused-index INFO remains. Claude's SG `clingen_vcep` 500 report did not reproduce with the exact frontend gene/cDNA/null-transcript lazy-section body (local and SG both 200), but Codex added a narrow lazy-section guard/test for legacy nullable ACMG criterion warnings. Verification: focused lookup-section + variant-library/Supabase migration suites passed, full backend `python -m pytest -q` passed, Ruff/Black/diff-check passed on touched backend files, SG `/healthz` 200, SG `POST /api/v1/lookup/sections` `clingen_vcep` 200/available, SG `computational_deep_dive` 200/missing with expected predictor-unavailable warnings, SG `GET /api/v1/library/popular` 200, and unauthenticated SG `GET /api/v1/library` 401. Local backend `:8000` was restarted earlier and now exposes `/api/v1/lookup` and `/api/v1/library`. No frontend/config/docs lane edits beyond this handoff heartbeat; `supabase/.temp/` is generated CLI noise and must not be committed.
...
UNLOCKED · 2026-06-07 00:38 +1000 · Claude (wrote → CODEX align-engine handoff + Align-v2 heartbeat; committing FE align pathspecs only — Codex `app/backend/**` WIP untouched; re-read clean)
