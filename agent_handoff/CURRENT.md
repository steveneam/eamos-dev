...
- **Codex:** IDLE @ 2026-06-11 21:29 +10:00 - Combined Workbench/PubMed integration commit approved by Steven and staged with explicit pathspecs only. Verification green on the combined tree: Workbench+health pytest, PubMed/health/tool pytest, backend Ruff, scoped Black checks, Workbench CRISPR index CLI help, app/frontend tsc, app/web tsc, and `git diff --cached --check`. Included shared `PROGRESS.md`, `agent_handoff/CURRENT.md`, and refreshed `graphify-out/*`. Excluded `.claude/settings.json`, `codex-workbench-temp.md`, and all generated/private CRISPR off-target SQLite/genome/index artifacts. Deploy guardrail: keep `CRISPR_OFFTARGET_PROVIDER=auto` unless Render has a real local `CRISPR_OFFTARGET_INDEX_PATH` and provider-cache health reports the index ready.
...
UNLOCKED - 2026-06-11 21:29 +10:00 - Codex (combined Workbench/PubMed integration staged with explicit pathspecs; verification green; no deploy; CRISPR indexed provider remains env-gated)
...
Section last edited: 2026-06-11 18:34 +10:00 - Codex. Detailed history is in PROGRESS.md and commit messages.

**Latest Codex update (2026-06-11 18:34 +10:00 - Codex):**
Steven resumed the PubMed lane after Claude frontend work and then launched a
separate Codex for the Workbench CAR. This Codex stayed on PubMed/PMC/Supabase
literature wiring and did not touch Workbench or Claude frontend WIP.

Completed:
- Added `python -m app.cli.eamos_pubmed_litvar_edges`, a no-network
  operator CLI that converts staged LitVar/LitVar2 publication JSON/JSONL
  exports into the existing PubMed-local literature-edge JSONL shape.
- Added tests proving sanitized converter output and end-to-end ingestion by
  ...
  publications.
- Updated `app/backend/app/services/build_ledger.py` so the literature engine
  describes SQLite as the proof harness and private Supabase Storage/Postgres
  as the planned production corpus tier after explicit sizing approval. API
  calls remain fallback/refresh.
- Updated `docs/pubmed-local/plan.md` with small local-proof buckets and a
  separate Supabase production track: `SUPA-LIT-0` inventory/size budget,
  then gated PubMed upload, PubTator selector import, filtered PMC OA, private
  tables, runtime adapter switch, and later vector/search upgrade.
- Checked current Supabase facts without mutation: org `Eamos` is Pro; project
  `eamos-dev` is healthy; Postgres is about 19 MB; WAL is about 80 MB; private
  Storage is about 38 GB in `eamos-source-assets`; bucket is private with
  60 GiB per-object limit.
- Checked current source-package scale from official listings: PubMed 2026
  baseline is about 50.6 GiB compressed plus about 7.0 GiB update files;
  PubTator3 full BioC XML is about 200 GiB compressed, while selector tables
  are much smaller; PMC OA XML baseline is about 135 GiB compressed and
  XML+text listed packages are about 258 GiB compressed.

Recommendation:
- Use Option A: PubMed-first filtered-source path. Run `SUPA-LIT-0`, stage on
  an explicit data volume rather than `C:`, filter/derive the Eamos corpus, and
  upload only approved source packages plus queryable metadata/edges.
- Stop for Steven approval before uploading PubMed baseline/update to
  Supabase, before any PMC/PubTator full-source mirror, before any Storage or
  database overage, and before vector bucket/embedding expansion.

Verification:
- `cd app/backend && python -m pytest tests\test_health_api.py tests\test_pubmed_litvar_edges.py tests\test_pubmed_pubtator_edges.py tests\test_pubmed_local.py -q`
  -> passed.
- `cd app/backend && python -m ruff check app\services\build_ledger.py app\cli\eamos_pubmed_litvar_edges.py tests\test_pubmed_litvar_edges.py`
  -> passed.
- `cd app/backend && python -m black --check --target-version py310 app\services\build_ledger.py app\cli\eamos_pubmed_litvar_edges.py tests\test_pubmed_litvar_edges.py`
  -> passed.
- `cd app/backend && python -m pytest tests\test_pubmed_litvar_edges.py tests\test_pubmed_pubtator_edges.py tests\test_pubmed_local.py tests\test_health_api.py tests\test_publication_literature.py tests\test_lookup_section_fetch_contract.py tests\test_variant_cache.py tests\test_frontend_contract.py -q`
  -> passed.
- `python -m graphify update .` -> passed; graph HTML skipped because graph
  exceeds the visualization node limit.

Commit/deploy state:
- No commit, push, deploy, Supabase upload, schema migration, or Storage
  mutation was performed in this slice.
- Worktree includes Codex PubMed files (`app/backend/app/cli/eamos_pubmed_litvar_edges.py`,
  `app/backend/tests/test_pubmed_litvar_edges.py`, `app/backend/app/services/build_ledger.py`,
  `docs/pubmed-local/plan.md`, `PROGRESS.md`, `agent_handoff/CURRENT.md`,
  graphify outputs) plus pre-existing/parallel `.claude/settings.json`.
- A separate Codex is working the Workbench CAR. Do not overwrite its
  Workbench status or edit Workbench files from the PubMed lane.
- Never use `git add -A`.

**Clear-safe:** yes for this PubMed slice. Handoff/progress are updated and no
background process was started by this Codex. The PubMed changes are verified
but uncommitted.

**Latest resume prompt:**
```text
# Resume prompt - 2026-06-11 18:34 +1000 - Codex PubMed LitVar + Supabase corpus plan
Eamos. Continue in D:\eamos on Windows/PowerShell only. Read CODEX.md, AGENTS.md, agent_handoff/README.md, agent_handoff/CURRENT.md, PROGRESS.md, docs/pubmed-local/{design,spec,plan}.md, and docs/proprietary/ep-vlex.md, then run git status --short --branch.

Delta: PubMed-local v4 edge ingestion was already live from `59feb38`. This slice adds `python -m app.cli.eamos_pubmed_litvar_edges` plus tests, updates the build ledger to identify Supabase private Storage/Postgres as the planned production corpus tier after sizing approval, and updates `docs/pubmed-local/plan.md` with local proof buckets and approval-gated Supabase buckets.

Supabase facts checked without mutation: org `Eamos` is Pro; project `eamos-dev` healthy; Postgres about 19 MB; WAL about 80 MB; private Storage about 38 GB in `eamos-source-assets`. Supabase Pro includes 100 GB Storage org-level and 8 GB database disk/project before overage. Current source scale from listings: PubMed baseline about 50.6 GiB compressed + updates about 7.0 GiB; PubTator3 full BioC XML about 200 GiB; PMC OA XML baseline about 135 GiB, XML+text listed packages about 258 GiB.

Recommended next: `SUPA-LIT-0` corpus inventory and size budget. No uploads in that bucket. Use an explicit staging volume, not `C:`. Stop for Steven approval before bulk Supabase upload, Storage/database overage, PMC/PubTator full-source mirroring, or vector/embedding expansion.

Verification already passed: targeted PubMed/health pytest, ruff, black, broader publication/lookup/frontend-contract pytest, and `python -m graphify update .`.

Coordination: a separate Codex is working the Workbench CAR. PubMed lane must not overwrite Workbench handoff/status or edit Workbench files unless Steven redirects. Guardrails: no `git add -A`; preserve live fallback/refresh and no-startup-download policy; no Supabase mutation without explicit approval.
End clear-safe.
```
