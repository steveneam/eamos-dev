...
- **Codex:** IDLE @ 2026-06-07 19:31 +1000 - Workbench backend gate is green for coordinated Claude commit path A: off-target backend, screening primers, ssODN donor-design endpoint, contract mirrors, compact-index materialization checks, and predictor adapter materialization inspections are verified. No commit/push/deploy by Codex this turn; after Claude pushes, Codex owns SG Render deploy hook plus live verification for `/api/v1/crispr/offtargets` and `/api/v1/crispr/screening-primers`.
...
Section last edited: 2026-06-07 19:31 +1000 - Codex. Detailed history is in PROGRESS.md and commit messages.

**Latest Codex update (2026-06-07 19:31 +1000 - Codex):**
Workbench backend is green for coordinated commit path A. Claude can commit the full coordinated Workbench/off-target FE + backend tree and push; Codex owns the SG Render deploy hook and live verification after push.

Completed Workbench backend surfaces:
- `POST /api/v1/crispr/offtargets` returns exact `{genome_build, sites}` with deterministic de-identified fixture coverage.
- `POST /api/v1/crispr/screening-primers` reuses the primer provider and returns Tm, GC, product size, specificity hits, other products, secondary-structure risk/notes, and `recommended`.
- `POST /api/v1/crispr/ssodn` is additive and returns orderable 5-prime-to-3-prime donor sequence, strand/orientation, variant offset, arm lengths, intron mask, warnings, and optional guide/PAM-block behavior.
- The RPE65 ssODN workbook was read only. All seven public examples match backend-generated uppercase donors exactly by content and variant position after ignoring manual workbook casing. No raw workbook sequences or CMRI-derived rows were committed.
- Both backend.ts mirrors are byte-identical.

Adapter materialization progress:
- Compact coordinate index remains explicit off-startup and verified through materialization/preflight/health tests.
- AlphaMissense and ESM1b materialization metadata paths have focused runtime tests.
- CI-SpliceAI and CAPICE now have admin-lane materialization inspectors for model/reference/score-cache or model/feature-cache state. Health, source-asset preflight, and build ledger use those inspectors while preserving launch-gate metadata and hiding local paths.

Verification:
- `python -m py_compile app\backend\app\services\predictor_runtime.py app\backend\app\api\routes\health.py app\backend\app\cli\eamos_source_asset_preflight.py app\backend\app\services\build_ledger.py app\backend\app\core\config.py`
- `python -m pytest tests\test_predictor_runtime.py tests\test_health_api.py::test_provider_cache_health_returns_sanitized_empty_aggregates tests\test_health_api.py::test_provider_cache_health_reports_ready_admin_predictors_without_paths tests\test_source_asset_preflight_cli.py::test_source_asset_preflight_reports_guarded_readiness tests\test_source_asset_preflight_cli.py::test_source_asset_preflight_reports_ready_admin_predictors_without_paths -q`
- `python -m black --check --target-version py310 ...` on touched predictor/health/preflight/build-ledger/test files.
- `python -m ruff check ...` on touched predictor/health/preflight/build-ledger/test files.
- `python -m pytest tests\test_workbench_api.py tests\test_frontend_contract.py tests\test_compact_coordinate_index_materialization.py tests\test_predictor_runtime.py tests\test_predictor_lane_scaffolds.py tests\test_health_api.py tests\test_source_asset_preflight_cli.py -q`
- backend.ts mirror byte check passed.
- `python -m graphify update .` passed.

Coordination answer for Claude:
- **A, green.** Do not stash ssODN/Align/compact-index WIP. Commit the full coordinated tree if Steven approves/Claude is executing that handoff. After push, Codex should fire the SG Render deploy hook and live-verify `POST /api/v1/crispr/offtargets` and `POST /api/v1/crispr/screening-primers`.

Next-session direction:
- If resumed after Claude push: deploy SG, then live-verify the two CRISPR off-target endpoints on SG.
- Then continue adapter materialization after the current checkpoint: PubMed/PMC local from `docs/pubmed-local/{design,spec,plan}.md`, with explicit CLI materialization/preflight only, no startup/build/predeploy downloads, live E-utilities fallback/refresh, provenance/checksum/de-identified metadata, and PMC/full text only where license/reuse permits.
- Preserve Claude FE WIP ownership unless explicitly redirected; stage explicit pathspecs only.

**Clear-safe:** yes. Work is uncommitted WIP only; no push/deploy/destructive git by Codex; active Codex locks released. The tree remains dirty with mixed Codex/Claude lanes, so stage only explicit pathspecs.

**Latest resume prompt:**
```text
# Resume prompt - 2026-06-07 19:31 +1000 - Codex after coordinated Workbench commit
Eamos. Continue in `D:\eamos` on Windows/PowerShell only. Read CODEX.md, AGENTS.md, agent_handoff/README.md, agent_handoff/CURRENT.md, docs/crispr-offtarget-screening/spec.md, docs/crispr-ssodn/spec.md, docs/workbench-align-engine/spec.md, docs/pubmed-local/{design,spec,plan}.md, then run `git status --short --branch`.

Use Graphify first for codebase questions: `python -m graphify query "<question>"`; after code changes run `python -m graphify update .`.

Current coordination state: Workbench backend is green for path A. Claude can commit the full coordinated Workbench/off-target FE + backend tree and push if Steven approves. Codex did not commit/push/deploy. After Claude pushes, Codex owns SG Render deploy hook and live verification for `POST /api/v1/crispr/offtargets` and `POST /api/v1/crispr/screening-primers`.

Backend state: CRISPR off-targets, screening primers, and dedicated ssODN donor design endpoint are implemented as additive Workbench backend contracts. The RPE65 workbook was checked read-only; all seven public 120 nt examples match backend uppercase donor content and variant position when manual casing is ignored. Do not commit the CMRI workbook or raw derived rows. Both backend.ts mirrors were byte-identical before handoff.

Adapter state: compact coordinate index explicit off-startup materialization is verified; AlphaMissense/ESM1b runtime materialization tests exist; CI-SpliceAI and CAPICE materialization inspectors now feed health/preflight/build-ledger and preserve launch_gate metadata.

Verification green before handoff: focused Workbench/backend pytest, frontend contract pytest, compact-index materialization pytest, predictor runtime/lane pytest, health/preflight pytest, Ruff, scoped Black check, backend.ts mirror byte check, and `python -m graphify update .`.

Next after deploy verification: continue PubMed/PMC local from docs/pubmed-local, with explicit CLI materialization/preflight only, no startup/build/predeploy downloads, live E-utilities fallback/refresh, provenance/checksum/de-identified metadata, and PMC/full text only where license/reuse permits. Preserve Claude FE WIP ownership unless explicitly redirected.
